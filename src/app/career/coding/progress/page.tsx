...
export default function ProgressDashboard() {
  ...
  return (
    <div className="max-w-6xl mx-auto py-8 px-4 space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="neu-card rounded-2xl p-6">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h3 className="text-slate-700 dark:text-white/70 font-medium">Study Plan Completion</h3>
          </div>
          <div className="mt-4 flex items-end gap-2 mb-3">
            ...
            <span className="text-slate-500 dark:text-slate-400 mb-1">/ {totalCount} problems</span>
          </div>
        ...
        <div className="neu-card rounded-2xl p-6 flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <Calendar className="w-5 h-5 text-orange-500 dark:text-orange-400" />
              <h3 className="text-slate-700 dark:text-white/70 font-medium">Daily Challenge</h3>
            </div>
            {dailyProblem ? (
              <div className="mt-4">
                <a
                ...
                <span className={`text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded border mt-2 inline-block ${
                  dailyProblem.difficulty === 'Easy' ? 'text-green-700 dark:text-green-400 border-green-300 dark:border-green-400/20 bg-green-100 dark:bg-green-400/10' :
                  dailyProblem.difficulty === 'Medium' ? 'text-yellow-700 dark:text-yellow-400 border-yellow-300 dark:border-yellow-400/20 bg-yellow-100 dark:bg-yellow-400/10' :
                  'text-red-700 dark:text-red-400 border-red-300 dark:border-red-400/20 bg-red-100 dark:bg-red-400/10'
                }`}>
                  {dailyProblem.difficulty}
                </span>
              </div>
            ) : (
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-4">Unable to load daily problem.</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="neu-card rounded-2xl p-6 h-80">
          <h3 className="text-slate-700 dark:text-white/70 font-medium mb-6">Total Solved (LeetCode)</h3>
          {diffData.length > 0 ? (
            ...
            <div className="h-full flex items-center justify-center text-slate-400 dark:text-white/30">No data</div>
          )}
        </div>

        <div className="neu-card rounded-2xl p-6 overflow-hidden flex flex-col">
          <h3 className="text-slate-700 dark:text-white/70 font-medium mb-4">Recently Completed (Study Plan)</h3>
          <div className="flex-grow overflow-y-auto custom-scrollbar pr-2 space-y-2">
            {completedHistory.length === 0 ? (
              <p className="text-sm text-slate-400 dark:text-white/30 text-center mt-10">No problems completed yet.</p>
            ) : (
              completedHistory.sort((a,b) => new Date(b.completed_at).getTime() - new Date(a.completed_at).getTime()).map(r => (
                <div key={r.id} className="neu-raised-sm flex justify-between items-center p-3 rounded-xl">
                  <div className="overflow-hidden pr-4">
                    <p className="text-sm text-slate-900 dark:text-white/90 font-medium truncate">{r.problem_title}</p>
                    <p className="text-xs text-indigo-600 dark:text-indigo-400 capitalize">{r.topic.replace(/-/g, ' ')}</p>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <p className="text-xs text-slate-500 dark:text-slate-400">{new Date(r.completed_at).toLocaleDateString()}</p>
                  </div>
                </div>
              ))
            )}
